...
#ifndef msr_airlib_PX4DroneController_hpp
#define msr_airlib_PX4DroneController_hpp
...
class PX4DroneController : public DroneControllerBase
{
public:
...
public:
    //required for pimpl
    PX4DroneController();
    virtual ~PX4DroneController();

    //non-base interface specific to PX4DroneController
...
struct PX4DroneController::impl {
public:
    ...
    PX4DroneController* parent_;

    impl(PX4DroneController* parent)
        : parent_(parent)
    {
    }
        ...
        std::shared_ptr<mavlinkcom::MavLinkNode>& node, std::shared_ptr<mavlinkcom::MavLinkConnection>& connection)
    {
        if (connection_ == nullptr)
            throw std::domain_error("PX4DroneController requires connection object to be set before createProxy call");
...
PX4DroneController::PX4DroneController()
{
...
void PX4DroneController::initialize(const ConnectionInfo& connection_info, const SensorCollection* sensors, bool is_simulation)
{
    pimpl_->initialize(connection_info, sensors, is_simulation);
}

PX4DroneController::ConnectionInfo PX4DroneController::getMavConnectionInfo()
{
    return pimpl_->getMavConnectionInfo();
}
void PX4DroneController::sendImage(unsigned char data[], uint32_t length, uint16_t width, uint16_t height)
{
    pimpl_->sendImage(data, length, width, height);
}

bool PX4DroneController::hasVideoRequest()
{
    return pimpl_->hasVideoRequest();
}
std::string PX4DroneController::findPX4()
{
    return impl::findPX4();
}
...
//*** Start: VehicleControllerBase implementation ***//
void PX4DroneController::reset()
{
    DroneControllerBase::reset();
    pimpl_->reset();
}
void PX4DroneController::update()
{
    DroneControllerBase::update();
    pimpl_->update();
}
real_T PX4DroneController::getVertexControlSignal(unsigned int rotor_index)
{
    return pimpl_->getVertexControlSignal(rotor_index);
}
size_t PX4DroneController::getVertexCount()
{
    return impl::RotorControlsCount;
}
void PX4DroneController::getStatusMessages(std::vector<std::string>& messages)
{
    pimpl_->getStatusMessages(messages);
}
bool PX4DroneController::isAvailable(std::string& message)
{
...
//DroneControlBase
Kinematics::State PX4DroneController::getKinematicsEstimated()
{
    return pimpl_->getKinematicsEstimated();
}

Vector3r PX4DroneController::getPosition()
{
    return pimpl_->getPosition();
}

Vector3r PX4DroneController::getVelocity()
{
    return pimpl_->getVelocity();
}

Quaternionr PX4DroneController::getOrientation()
{
    return pimpl_->getOrientation();
}

GeoPoint PX4DroneController::getHomeGeoPoint()
{
    return pimpl_->getHomeGeoPoint();
}

GeoPoint PX4DroneController::getGpsLocation()
{
    return pimpl_->getGpsLocation();
}

DroneControllerBase::LandedState PX4DroneController::getLandedState()
{
    return pimpl_->getLandedState();
}
//administrative

bool PX4DroneController::armDisarm(bool arm, CancelableBase& cancelable_action)
{
    return pimpl_->armDisarm(arm, cancelable_action);
}


void PX4DroneController::enableApiControl(bool is_enabled)
{
    pimpl_->enableApiControl(is_enabled);
}
void PX4DroneController::setSimulationMode(bool is_set)
{
    pimpl_->setSimulationMode(is_set);
}
bool PX4DroneController::isApiControlEnabled()
{
    return pimpl_->isApiControlEnabled();
}
bool PX4DroneController::isSimulationMode()
{
    return pimpl_->isSimulationMode();
}

bool PX4DroneController::takeoff(float max_wait_seconds, CancelableBase& cancelable_action)
{
    return pimpl_->takeoff(max_wait_seconds, cancelable_action);
}

bool PX4DroneController::hover(CancelableBase& cancelable_action)
{
    return pimpl_->hover(cancelable_action);
}

bool PX4DroneController::land(float max_wait_seconds, CancelableBase& cancelable_action)
{
    return pimpl_->land(max_wait_seconds, cancelable_action);
}

bool PX4DroneController::goHome(CancelableBase& cancelable_action)
{
    return pimpl_->goHome(cancelable_action);
}

void PX4DroneController::commandRollPitchZ(float pitch, float roll, float z, float yaw)
{
    return pimpl_->commandRollPitchZ(pitch, roll, z, yaw);
}
void PX4DroneController::commandVelocity(float vx, float vy, float vz, const YawMode& yaw_mode)
{
    return pimpl_->commandVelocity(vx, vy, vz, yaw_mode);
}
void PX4DroneController::commandVelocityZ(float vx, float vy, float z, const YawMode& yaw_mode)
{
    return pimpl_->commandVelocityZ(vx, vy, z, yaw_mode);
}
void PX4DroneController::commandPosition(float x, float y, float z, const YawMode& yaw_mode)
{
    return pimpl_->commandPosition(x, y, z, yaw_mode);
}

RCData PX4DroneController::getRCData()
{
    return pimpl_->getRCData();
}
void PX4DroneController::setRCData(const RCData& rcData)
{
    return pimpl_->setRCData(rcData);
}

bool PX4DroneController::loopCommandPre()
{
    return pimpl_->loopCommandPre();
}

void PX4DroneController::loopCommandPost()
{
    pimpl_->loopCommandPost();
}

//drone parameters
float PX4DroneController::getCommandPeriod()
{
    return pimpl_->getCommandPeriod();
}
float PX4DroneController::getTakeoffZ()
{
    return pimpl_->getTakeoffZ();
}
float PX4DroneController::getDistanceAccuracy()
{
    return pimpl_->getDistanceAccuracy();
}
const VehicleParams& PX4DroneController::getVehicleParams()
{
    return pimpl_->getVehicleParams();
}
//TODO: decouple DroneControllerBase, VehicalParams and SafetyEval

void PX4DroneController::reportTelemetry(float renderTime)
{
    return pimpl_->reportTelemetry(renderTime);
}

Pose PX4DroneController::getDebugPose()
{
